...
indexes_to_persian = ['آهسته', 'آمدن', 'برادر', 'باید', 'بیمار', 'بیمارستان', 'چند', 'چطور', 'چی', 'دانشگاه', 'دارو',
                      'داشتن', 'دیدن', 'دکتر', 'دختر', 'درسته', 'امروز', 'امشب', 'فهمیدن', 'قبل', 'گفتن', 'هفته',
                      'حتما', 'کی (چه زمانی)',
                      'خواهر', 'خوب', 'خوشحال', 'کجا', 'مادر', 'من', 'میخواهم', 'معلم', 'ناشنوا', 'اون', 'پدر', 'پسر',
                      'رفتن', 'روز', 'شنوا', 'واقعا']

indexes_in_clsmap = {value: key for value, key in enumerate(indexes_to_persian)}
...
def set_prediction_callback(callback):
    global _prediction_callback
    _prediction_callback = callback
...
def run(show=False):
    ...
    try:
        while True:
            ...
            pose_raw_data = np.zeros((33, 3), dtype=np.float32)
            ...
            feature_data = compute_and_norm_landmarks(left_hand_filtered, right_hand_filtered, pose_raw_data)
            ...
            if len(frame_60_sequence) == config.SEQUENCE_LENGTH and moz_counter % 3 == 0:
                sequence = np.array(frame_60_sequence, dtype=np.float32)  # (60, 180)
                # adds the one which shows that we give ONE input at a time
                sequence = np.expand_dims(sequence, axis=0)  # (1, 60, 180)
            ...
            if _prediction_callback is not None and current_confidence * 100 > 75:
                _prediction_callback(
                    current_prediction,
                    current_confidence,
                    frame
                )
